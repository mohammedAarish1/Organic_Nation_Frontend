...
const FormField = ({
  ...
  const { userCity, userPincode, userState, error } = useSelector(
  ...
  return (
    <div>
      ...
      <Field
        ...
        className={`w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[var(--accent-color)] shadow-sm ${
...
const NewCheckoutForm = ({ close }) => {
  ...
  const { user } = useSelector((state) => state.auth);
  const { cartItemsList, totalCartAmount, totalTax, couponCodeApplied } =
  ...
  const { discountAmount, taxDiscount } = additionalDiscountforOnlinePayment(
    totalCartAmount,
    totalTax
  );
  const [isTestingProduct, setIsTestingProduct] = useState(false);
  // const isTestingProduct=cartItemsList?.some(product=>product?.category==='Demo')
    ...
    try {
      ...
      const checkoutData = {
        ...
        merchantTransactionId:
          paymentMethod === "cash_on_delivery" ? "" : merchantTransactionId,
  ...
  const handleSubmit = (values, action) => {
    ...
    const shippingInfo = {
      ...
      email: values.email,
      phoneNumber: values.phoneNumber.includes("+91")
        ? values.phoneNumber
  ...
  const handleSavedAddressCheckout = () => {
    ...
    const addressToUse = savedAddresses.find(
  ...
  useEffect(() => {
    const value = cartItemsList?.some(
      (product) => product?.category === "Demo"
    );
    setIsTestingProduct(value)
  }, [isTestingProduct,cartItemsList]);
