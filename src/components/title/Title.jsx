...
const Title = ({ 
  ...
  // Component wrapper - use div if animations disabled
  const Component = animationDisabled ? 'div' : motion.div;
  const HeadingComponent = animationDisabled ? 'h2' : motion.h2;
  const SubComponent = animationDisabled ? 'p' : motion.p;
  const LineComponent = animationDisabled ? 'div' : motion.div;

  const animationProps = animationDisabled ? {} : {
    variants: containerVariants,
    initial: "hidden",
    animate: "visible"
  };
  ...
  return (
    <Component 
      className={`flex flex-col items-center my-8 sm:my-12 ${className}`}
      {...animationProps}
    >
      {/* Main Heading */}
      <HeadingComponent
        className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-center mb-4 relative group"
        style={{ color: 'var(--themeColor, #1f2937)' }}
        {...(!animationDisabled && { variants: headingVariants })}
      >
        {heading}
        
        {/* Simple hover underline */}
        <div 
          className="absolute -bottom-2 left-1/2 transform -translate-x-1/2 h-0.5 bg-gradient-to-r from-transparent via-current to-transparent opacity-0 group-hover:opacity-60 transition-opacity duration-300 w-0 group-hover:w-full transition-all"
      ...
      <div className="flex items-center gap-3 mb-4">
        <LineComponent
          className="h-px w-12 bg-gradient-to-r from-transparent to-current opacity-60"
          style={{ color: 'var(--accent-color, #3b82f6)' }}
          {...(!animationDisabled && { variants: lineVariants })}
        />
        ...
        <LineComponent
          className="h-px w-12 bg-gradient-to-l from-transparent to-current opacity-60"
          style={{ color: 'var(--accent-color, #3b82f6)' }}
          {...(!animationDisabled && { variants: lineVariants })}
        />
      </div>
      ...
      {subHeading && (
        <SubComponent
          className="text-sm sm:text-base md:text-lg text-center max-w-2xl px-4 leading-relaxed opacity-80"
          style={{ color: 'var(--text-color, #6b7280)' }}
          {...(!animationDisabled && { variants: subHeadingVariants })}
        >
          {subHeading}
        </SubComponent>
      )}
