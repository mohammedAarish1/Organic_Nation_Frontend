...
const CompleteOrderModal = ({ isOpen, onClose, order, }) => {
  ...
  useEffect(() => {
    if (order?.paymentMethod === 'online_payment' && order?.paymentStatus === 'pending') {
  ...
  const handleSubmit = (values, action) => {
    handleOrderCompletion(values.paymentMethod)
      .then(() => {
            ...
            <div className="bg-[var(--neutral-color)]/30 rounded-2xl p-5 border border-[var(--neutral-color)]">
              ...
              <div className="space-y-3 text-sm">
                ...
                <div className="flex justify-between items-center border-[var(--neutral-color)]">
                  <span className="font-bold text-[var(--text-color)]">Total Amount:</span>
            ...
            <div className="space-y-4">
              <h3 className="font-semibold text-[var(--text-color)] mb-4 flex items-center gap-2">
                <div className="w-6 h-6 bg-[var(--themeColor)] rounded-full flex items-center justify-center">
                  <span className="text-[var(--text-light-color)] text-xs font-bold">₹</span>
                </div>
...
const IncompleteOrder = ({ onDismiss }) => {
  ...
  const handleDismiss = () => {
    setIsVisible(false);
    if (incompleteOrder) {
      sessionStorage.setItem(`incomplete-payment-dismissed-${incompleteOrder._id}`, 'true');
    }
    onDismiss?.();
  };
            ...
            <div className="flex items-start justify-between mb-3">
              <div>
                ...
                <p className="text-[#3E2C1B]/70 text-sm">
                  {itemCount} item{itemCount !== 1 ? 's' : ''} • ₹ {getOriginalAmt(incompleteOrder.subTotal)} <span className='text-[var(--themeColor)]'>(Grab 5% off on instant payment)</span>
                </p>
              </div>
              <div className="flex items-center space-x-1 bg-[#D87C45]/10 px-2 py-1 rounded-full">
                ...
                <span className="text-[#D87C45] text-xs font-medium">Awaiting Payment</span>
              </div>
            </div>
              ...
              <div className="space-y-1 max-h-20 overflow-y-auto">
                {incompleteOrder.orderDetails?.map((item, idx) => (
                  <div key={idx} className="flex justify-between text-sm">
                    <span className="text-gray-700 truncate capitalize">{item['name-url'].replaceAll('-', ' ')}</span>
                    <span className="text-gray-900 font-medium">{item.quantity} Pcs. </span>
