...
const ProductImageSection = memo(
  ({
    ...
    const frontImageIndex = product?.img?.findIndex(img =>
  Object.values(img).some(
    path => typeof path === "string" && path.toLowerCase().includes("front")
  )
);

useEffect(() => {
  if (frontImageIndex !== -1) {
    setSelectedImage(frontImageIndex);
  }
}, [frontImageIndex]);
      ...
      <motion.div variants={itemVariants} className="">
        ...
        <div className="relative">
          <motion.div
            ...
            onClick={() => {
              setFullScreenImages(product.img.map((img) => img.lg));
