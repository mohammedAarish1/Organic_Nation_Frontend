...
export const spotlightProducts = createSlice({
    ...
    reducers: {
        getSpotlightProducts: (state, action) => {
            ...
            // Create a dynamic key based on the product type
            const key = type && `meta.${type}`;
            if (key) {
                const filteredProducts = products?.filter((curItem) => curItem.meta[type]);
                state.spotlightProducts = filteredProducts;
